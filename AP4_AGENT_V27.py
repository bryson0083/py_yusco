...
def Chk_AP4():
	...
	if cnt == 0:
		if recnt < 3:
			print("$$$ V27: AP4出現異常，執行重啟程序. $$$\n")
			file.write("$$$ V27: AP4出現異常，執行重啟程序. $$$\n\n")
...
def MAIN_CHK_AP4():
	...
	p = multiprocessing.Process(target=Chk_AP4, name="Chk_AP4", args=())
	p.start()

	# Wait a maximum of 180 seconds for Chk_AP4
	# Usage: join([timeout in seconds])
	p.join(180)

	# If thread is active
	if p.is_alive():
		print(str_date + ": Timeout but Chk_AP4 is running... let's kill it...")
		file.write("PROG: AP4_AGENT_V27 Timeout.\n")
		file.write(str_date + ": Timeout but Chk_AP4 is running... let's kill it...\n\n")
